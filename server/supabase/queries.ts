...
export async function getDayEntries(date: string, uid?: string): Promise<FoodEntryRow[]> {
...
export async function getWeekEntries(endDate: string, uid?: string): Promise<FoodEntryRow[]> {
...
export async function getRecentWeightLogs(limit = 7, uid?: string): Promise<WeightLogRow[]> {
  const { data, error } = await supabase
    .from("weight_logs")
    .select("*")
    .eq("user_id", userId(uid))
