...
export const getDailySummary = {
  ...
  async execute({ date }: { date?: string }) {
    ...
    const adherence = Math.round(
      Math.max(0, 100 - (Math.abs(remaining.calories) / Math.max(targets.calories, 1)) * 120) *
        0.6 +
    ...
    if (remaining.calories < 0) {
      ...
      coachNote =
        "Still early in the day. A bigger, fiber-heavy lunch would keep the evening calmer.";
    }
