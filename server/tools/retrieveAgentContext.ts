import { z } from "zod";
import { getDayEntries, getGoals, getPreferences, getMemoryFacts } from "../supabase/queries.js";
  ...
  async execute({ date }: { date?: string }) {
    ...
    const prefsText = prefsRow
      ? `🍽️ Preferences: ${prefsRow.diet_type ?? "standard"} diet` +
        (prefsRow.dietary_restrictions.length > 0
          ? `, restrictions: ${prefsRow.dietary_restrictions.join(", ")}`
          : "") +
        (prefsRow.cuisine_preferences.length > 0
          ? `, cuisines: ${prefsRow.cuisine_preferences.join(", ")}`
          : "") +
    ...
    const mealsText =
      entries.length > 0
        ? `🥗 Today's meals (${entries.length}):\n` +
          entries
            .map((e) => `   • ${e.display_name ?? e.food_name} (${e.meal}) — ${e.calories} kcal`)
            .join("\n")
