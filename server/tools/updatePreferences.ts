import { z } from "zod";
import { upsertPreferences, getPreferences } from "../supabase/queries.js";

export const updatePreferencesInput = {
    dietary_restrictions: z.array(z.string()).optional(),
    cuisine_preferences: z.array(z.string()).optional(),
    diet_type: z.enum(["standard", "keto", "paleo", "vegan", "vegetarian", "mediterranean", "high_protein", "low_carb"]).optional(),
...
export const updatePreferences = {
    ...
    async execute(input: {
        ...
        if (saved.dietary_restrictions.length > 0) parts.push(`Restrictions: ${saved.dietary_restrictions.join(", ")}`);
        if (saved.cuisine_preferences.length > 0) parts.push(`Cuisines: ${saved.cuisine_preferences.join(", ")}`);
