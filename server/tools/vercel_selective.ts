...
export const listProjects = {
    ...
    async execute({ limit }: { limit: number }) {
        ...
        return {
            content: [{ type: "text" as const, text: `Vercel Projects:\n${projects || "No projects found."}` }],
        };
    },
};
...
export const getRuntimeLogs = {
    ...
    async execute({ projectId, deploymentId, limit }: { projectId: string; deploymentId?: string; limit: number }) {
        ...
        if (!deployId) {
            return { content: [{ type: "text" as const, text: "No deployment found to fetch logs for." }] };
        }

        const logsData = await vercelFetch(`/v2/deployments/${deployId}/events?limit=${limit}`);
        const logLines = (Array.isArray(logsData) ? logsData : []).map((l: any) => `[${l.type}] ${l.payload?.text || l.payload?.message || ""}`).join("\n");

        return {
            content: [{ type: "text" as const, text: `Logs for ${deployId}:\n${logLines || "No logs found."}` }],
        };
    },
};
...
export const createDeployment = {
    ...
    async execute({ projectId, branch }: { projectId: string; branch?: string }) {
        return {
            content: [{ type: "text" as const, text: `Vercel: Deployment trigger for ${projectId} on branch ${branch || "default"} is scheduled. (API /v13/deployments)` }],
        };
    },
};
